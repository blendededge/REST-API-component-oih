...
const getValidMimeType = (contentType) => (ALLOWED_CONTENT_TYPES.includes(contentType) ? contentType : 'application/octet-stream');
...
module.exports.processMethod = async function (msg, cfg, snapshot, TOKEN) {
  ...
  try {
    ...
    const { auth: authConfig } = getAuthFromSecretConfig(cfg, emitter.logger);
    auth = authConfig;

    requestTimeout = getRequestTimeout(emitter.logger, cfg);
    ...
    rateLimitDelay = getRateLimitDelay(emitter.logger, cfg);
  ...
  async function fetchNewToken() {
    ...
    if (authTokenResponse.status >= 400) {
      throw new Error(
        `Error in authentication.  Status code: ${authTokenResponse.status
  ...
  async function buildErrorStructure(e) {
    ...
    if (
      ...
      emitter.logger.info('Starting rebound, Component error: ', JSON.stringify(e), JSON.stringify(addErrorLogContext(e)));
      ...
      emitter.logger.debug('Component output: ', JSON.stringify(returnOutput), JSON.stringify(addErrorLogContext(e)));
      await emitter.emit('data', messages.newMessage(returnOutput));
      await rateLimit(emitter.logger, rateLimitDelay);
    } else {
      emitter.logger.error('Component error: ', JSON.stringify(e), JSON.stringify(addErrorLogContext(e)));
